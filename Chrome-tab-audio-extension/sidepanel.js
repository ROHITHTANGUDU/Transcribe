...
// let fullChunks = [];  // continuous raw blobs for stitched full session
// let mimeHint = '';
// let runningFB = false; // picker fallback flag
// let fbInit = null;  // init blob in fallback continuous mode
// let fbRec = null;  // fallback continuous recorder
// let fbStream = null;
// let fbTimeslice = 10000;

// // Transcription variables
// const serverUrl = "http://127.0.0.1:8000"; // Your Python server URL
...
//   transcriptionDiv.style.whiteSpace = "pre-wrap";
...
//   // Use correct extension based on mimeHint
//   const ext = mimeHint.includes('wav') ? 'wav' : 'webm';
//   formData.append("file", blob, `audio_chunk.${ext}`);
    
//     const response = await fetch(`${serverUrl}/transcribe_chunk`, {
//       method: "POST",
//       body: formData
//     });
...
//         <p style="margin: 0.2rem 0 0 0;">${transcriptionBuffer.join(" ")}</p>
//       </div>
//     `;
//     transcriptionBuffer = [];
...
//   li.insertAdjacentHTML('beforeend', `<code>[${chunkList.children.length + 1}]</code> `);
//   const a = document.createElement('a');
//   a.href = url;
//   a.download = `chunk-${dt}.${ext}`;
...
//   if (p.type === 'STATE') {
//     if (p.state === 'RECORDING') statusEl.textContent = `Recording from: ${p.tabTitle || 'current tab'} (no picker)`;
//     if (p.state === 'IDLE') { buildAndShowFull(); startBtn.disabled = false; stopBtn.disabled = true; }
//   }

//   if (p.type === 'CHUNK') {
//     mimeHint = p.mimeType || mimeHint || p.blob?.type || 'audio/webm';
//     fullChunks.push(p.blob);
//     // Automatically transcribe every chunk
//     transcribeChunk(p.blob);
//   }

//   if (p.type === 'SEGMENT') {
//     const type = p.mimeType || p.blob?.type || 'audio/webm';
//     addDownloadItem(p.blob, type);
//   }

//   if (p.type === 'NEED_PICKER') startPickerFallback();
//   if (p.type === 'ERROR') { statusEl.textContent = 'Auto-capture not available; switching to picker…'; startPickerFallback(); }
// });
...
//     fbStream = new MediaStream(tracks);
//     fbTimeslice = Math.max(2, Number(chunkSecsEl.value || 10)) * 1000;
//     runningFB = true; fbInit = null;
...
//     fbRec = new MediaRecorder(fbStream, mimeType ? { mimeType } : undefined);
//     fbRec.ondataavailable = (e) => {
//       if (!e.data || !e.data.size) return;
//       if (!fbInit) fbInit = e.data;
//       mimeHint = mimeType || mimeHint || e.data.type || 'audio/webm';
//       fullChunks.push(e.data);
//       // Transcribe chunks in fallback mode too
//       transcribeChunk(e.data);
//     };
//     fbRec.onstop = () => {
//       try { fbStream?.getTracks().forEach(t => t.stop()); } catch {}
//       fbStream = null; fbRec = null; runningFB = false;
//       statusEl.textContent = 'Stopped.'; buildAndShowFull();
//       startBtn.disabled = false; stopBtn.disabled = true;
//     };
//     fbRec.start(fbTimeslice);

//     const startSeg = () => {
//       if (!runningFB) return;
//       const segRec = new MediaRecorder(fbStream, mimeType ? { mimeType } : undefined);
//       let segBlob = null;
//       segRec.ondataavailable = (e) => { if (e.data && e.data.size) segBlob = e.data; };
//       segRec.onstop = () => { 
//         if (segBlob) addDownloadItem(segBlob, mimeType || segBlob.type); 
//         if (runningFB) startSeg(); 
//       };
//       segRec.start();
//       setTimeout(() => { if (segRec.state !== 'inactive') segRec.stop(); }, fbTimeslice);
//     };
//     startSeg();

//     statusEl.textContent = 'Recording (picker fallback)…';
//     startBtn.disabled = true; stopBtn.disabled = false;
//     setupTranscriptionDisplay();
//   } catch (e) {
//     statusEl.textContent = 'Error (picker): ' + (e?.message || e);
//     startBtn.disabled = false; stopBtn.disabled = true;
//   }
// }
...
//   if (runningFB) {
//     if (fbRec && fbRec.state !== 'inactive') fbRec.stop();
//   } else {
//     try { await chrome.runtime.sendMessage({ from: 'sidepanel', type: 'STOP' }); } catch {}
//   }
// });
...
// // NEW: per-chunk recorder loop state (fallback)
// let fbChunkIndex = 0;
...
//   try {
//     const formData = new FormData();
//     formData.append("file", blob, `audio_chunk.webm`); // always webm
...
//       console.warn("Transcription error:", result.error, result.details || "");
//     }
//   } catch (error) {
//     console.error("Transcription error:", error);
//   }
...
//   if (p.type === 'CHUNK') {
//     // NOTE: Offscreen path may still give headerless WebM fragments.
//     // Fallback path below fixes this by restarting per chunk.
//     mimeHint = p.mimeType || mimeHint || p.blob?.type || 'audio/webm';
//     fullChunks.push(p.blob);
//     transcribeChunk(p.blob);
//   }
...
// // --------- PICKER FALLBACK (fixed to ensure header per chunk) ----------
// async function startPickerFallback() {
//   try {
//     statusEl.textContent = 'Site blocks auto-capture. Select the tab and tick "Share tab audio."';
//     const ds = await navigator.mediaDevices.getDisplayMedia({ video: true, audio: true });
...
//       for (const t of types) if (MediaRecorder.isTypeSupported?.(t)) return t;
//       return ''; // let browser decide, Chrome will choose webm
//     };
//     const mimeType = pickWebmMime();

//     // Helper: start one chunk with a fresh recorder (guarantees EBML header)
//     const startOneChunk = (secs) => {
//       if (!runningFB) return;

//       // Safety: stop any previous
//       if (fbCurrentRec && fbCurrentRec.state !== 'inactive') {
//         try { fbCurrentRec.stop(); } catch {}
//       }

//       fbCurrentRec = new MediaRecorder(fbStream, mimeType ? { mimeType } : undefined);

//       let chunkBlob = null;
//       fbCurrentRec.ondataavailable = (e) => {
//         if (e.data && e.data.size) chunkBlob = e.data;
//       };

//       fbCurrentRec.onstop = () => {
//         if (chunkBlob) {
//           fbChunkIndex += 1;
//           const named = new File([chunkBlob], `chunk-${fbChunkIndex}.webm`, { type: chunkBlob.type || 'audio/webm' });
//           console.log('[FALLBACK CHUNK]', { index: fbChunkIndex, size: named.size, type: named.type });

//           addDownloadItem(named, named.type);
//           fullChunks.push(named);
//           transcribeChunk(named);
//         }
//         if (runningFB) {
//           // microtask delay avoids immediate restart issues
//           setTimeout(() => startOneChunk(secs), 0);
//         }
//       };

//       fbCurrentRec.start(); // no timeslice → full header written for this blob
...
//     // When user stops, we stop current recorder and release stream
//     stopBtn.onclick = () => {
//       if (!runningFB) return;
//       runningFB = false;
//       clearTimeout(fbChunkTimer);
//       try { if (fbCurrentRec && fbCurrentRec.state !== 'inactive') fbCurrentRec.stop(); } catch {}
//       try { fbStream?.getTracks().forEach(t => t.stop()); } catch {}
//       fbStream = null; fbCurrentRec = null; fbChunkTimer = null;
...
// startBtn.addEventListener('click', async () => { resetUI(); await startOffscreenFlow(); });
// stopBtn.addEventListener('click', async () => {
//   // If fallback is running, its own stopBtn.onclick handles cleanup
//   if (!runningFB) {
//     try { await chrome.runtime.sendMessage({ from: 'sidepanel', type: 'STOP' }); } catch {}
//   }
// });
...
// // ===== Sidepanel JS (WebM-only, per-chunk, nice transcript layout) =====
...
//   hdr.className = 'tx-header';
//   const ts = new Date().toLocaleTimeString();
//   hdr.textContent = `[${ts}] • ${source || currentSource || 'Unknown'}`;
...
// // ---------- offscreen path (may emit headerless blobs) ----------
// async function startOffscreenFlow() {
//   const chunkSecs = Math.max(2, Number(chunkSecsEl.value || 10));
//   statusEl.textContent = 'Capturing current active tab… (no picker on non-DRM sites)';
//   try {
...
//   if (p.type === 'CHUNK') {
//     // Offscreen may send fragments; fallback ensures headers per-chunk.
//     mimeHint = p.mimeType || mimeHint || p.blob?.type || 'audio/webm';
//     fullChunks.push(p.blob);
//     transcribeChunk(p.blob);
//   }
...
// // ---------- picker fallback (always standalone WebM per chunk) ----------
// async function startPickerFallback() {
//   try {
//     statusEl.textContent = 'Site blocks auto-capture. Select the tab and tick "Share tab audio."';
//     const ds = await navigator.mediaDevices.getDisplayMedia({ video: true, audio: true });
...
//       fbCurrentRec.start(); // no timeslice: ensures EBML header per blob
//       fbChunkTimer = setTimeout(() => {
//         if (fbCurrentRec && fbCurrentRec.state !== 'inactive') fbCurrentRec.stop();
//       }, Math.max(2, Number(chunkSecsEl.value || secs)) * 1000);
//     };
...
//     // In fallback, stopping is handled here:
//     stopBtn.onclick = () => {
//       if (!runningFB) return;
//       runningFB = false;
//       clearTimeout(fbChunkTimer);
...
// ===== Sidepanel JS (final) =====
// - WebM-only per-chunk recording (every chunk has EBML header)
// - Live transcript (timestamp • source)
// - Meeting timer + Copy/Download (.txt/.json)
// - **Stop** ends recording AND the Chrome **screen-share banner**
...
//   clearTranscriptionDisplay();
//   stopTimer();
//   const t = document.getElementById('meeting-timer'); if (t) t.textContent = '00:00:00';
...
//   const li = document.createElement('li');
//   li.insertAdjacentHTML('beforeend', `<code>[${chunkList.children.length + 1}]</code> `);
//   const a = document.createElement('a'); a.href = url; a.download = `chunk-${dt}.${ext}`;
//   a.textContent = `Download ${a.download} (${(blob.size/1024).toFixed(0)} KB)`; li.appendChild(a);
//   chunkList.appendChild(li);
// }
...
//     if (result.text) addTranscriptionText(result.text);
//   } catch (err) {
...
//   fbChunkIndex = 0;     // fallback recorder index
...
// // Minimal changes: keep archiving in fullChunks AND do in-order transcription with retries.
...
//   // Keep original behavior: archive every chunk so we can stitch and download later
...
//   // Give stable name for logs
//   const named = new File([chunkBlob], `session-${sessionId}_chunk-${fbChunkIndex}.webm`, { type: chunkBlob.type || 'audio/webm' });
...
//   // attempt = 0 is initial (no delay before it)
...
//         return true;
//       } else {
//         const msg = (result && (result.error || result.details)) || 'Empty transcription';
...
//   if (p.type === 'CHUNK') {
//     mimeHint = p.mimeType || mimeHint || p.blob?.type || 'audio/webm';
//     // minimal change: archive + enqueue FIFO
//     addFullChunk(p.blob);
//   }
...
//         // User-friendly suggestions depending on common cases
//         const tips = [];
...
//     // minimal change: archive + enqueue FIFO
//     addFullChunk(p.blob);
//     // Note: Download list is handled only in fallback path (SEGMENT),
//     // offscreen path can also add via SEGMENT if your background emits it.
//   }
...
//       fbCurrentRec.onstop = () => {
//         if (chunkBlob) {
//           // Download list + index handled here
//           addDownloadItem(chunkBlob, chunkBlob.type || 'audio/webm');
//           // Archive + enqueue for FIFO transcription
...
//   // NEW: reset health LED (safe if not defined yet)
...
//       // NEW (minimal): show ORANGE when sending to Python
...
//       // NEW (minimal): server OK but LLM failed → keep ORANGE
...
//         // NEW (minimal): terminal failure → RED
...
// /* ================== NEW: Health Light + Refresh (minimal add) ================== */
// (function addHealthStyles() {
//   if (document.getElementById('health-styles')) return;
//   const style = document.createElement('style');
//   style.id = 'health-styles';
//   style.textContent = `
//     .health-wrap { display:inline-flex; align-items:center; gap:8px; margin-left:10px; }
...
//   if (document.getElementById('health-controls')) return;
//   const wrap = document.createElement('span');
//   wrap.id = 'health-controls';
//   wrap.className = 'health-wrap';

//   const btn = document.createElement('button');
//   btn.className = 'health-btn';
//   btn.textContent = 'Refresh';
//   btn.onclick = () => runHealthCheck();

//   healthLedEl = document.createElement('span');
//   healthLedEl.className = 'health-led';
//   healthLedEl.title = 'Health status';

//   wrap.appendChild(btn);
//   wrap.appendChild(healthLedEl);

//   // place right next to Download full session link
//   dlEl.insertAdjacentElement('afterend', wrap);
// }
...
//     if (j.python_ok && j.llm_ok) { setHealthLight('green');  statusEl.textContent = 'Health: OK (Python+LLM)'; }
